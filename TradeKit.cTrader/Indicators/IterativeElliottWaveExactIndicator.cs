    ...
    public override void Calculate(int index)
    {
        ...
        if (IsLastBar)
        {
            ...
            for (int i = 0; i < mainPoints.Count - 1; i++)
            {
                ...
                if (innerPoints.All(p => p.BarIndex != start.BarIndex)) innerPoints.Insert(0, start);
                if (innerPoints.All(p => p.BarIndex != end.BarIndex)) innerPoints.Add(end);
                ...
                if (best != null)
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(res.NodeName)) continue;
