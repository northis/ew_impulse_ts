...
// Loads detected ABCDE-triangle setups from the backend and quizzes the user:
// show the chart up to the entry (trend + triangle correction), let them
// decide enter / skip, then reveal the TP/SL outcome and score the decision.
...
/** Parses "TRIANGLE_CONTRACTING fibo=0.83" from the finder comment. */
function parseComment(comment) {
    const m = (comment || '').match(/^(\S+)(?:\s+fibo=([\d.]+))?/);
    return {
...
async function runScan() {
    ...
    try {
        const res = await fetch('/api/triangle/scan', {
            method: 'POST',
