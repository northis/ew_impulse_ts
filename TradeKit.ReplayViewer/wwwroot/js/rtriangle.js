...
// Loads detected RUNNING ABCDE-triangle setups from the backend and quizzes the
// user: show the chart up to the entry (strong trend + triangle correction where
// wave B breaks point 0), let them decide enter / skip, then reveal the TP/SL
// outcome and score the decision. See EW_R_TRIANGLE.md.
...
function drawSetupBase(s) {
    ...
    // Triangle zigzag 0-A-B-C-D-E
    for (let i = 0; i + 1 < wp.length; i++)
        addLine(wp[i].bar, wp[i].price, wp[i + 1].bar, wp[i + 1].price, '#b78cf2', 2, 0);

    // Trendlines: A-C (extended to E's bar) and B-D
    if (wp.length >= 6) {
        const A = wp[1], B = wp[2], C = wp[3], D = wp[4], E = wp[5];
        const acSlope = (C.price - A.price) / Math.max(1, C.bar - A.bar);
        addLine(A.bar, A.price, E.bar, A.price + acSlope * (E.bar - A.bar), '#e0b84a', 1, 2);
        const bdSlope = (D.price - B.price) / Math.max(1, D.bar - B.bar);
        addLine(B.bar, B.price, E.bar, B.price + bdSlope * (E.bar - B.bar), '#e0b84a', 1, 2);
    }

    // Thrust leg (E → entry)
    if (wp.length >= 6)
        addLine(wp[5].bar, wp[5].price, s.entryBar, s.entryPrice, dir, 1, 2);
...
/** Parses "TRIANGLE_RUNNING run=1.42" from the finder comment. */
function parseComment(comment) {
    const m = (comment || '').match(/^(\S+)(?:\s+run=([\d.]+))?/);
    return {
        model: m && m[1] ? m[1].replace('TRIANGLE_', '').toLowerCase() : '—',
        run: m && m[2] ? m[2] : '—'
    };
}
...
function renderAnswer(s, decision) {
    ...
    // Outcome marker (appended to the wave-label markers set by drawSetupBase)
...
async function runScan() {
    ...
    const body = {
        ...
        takeProfitAtWaveB: $('pTpWaveB').checked,
        emitRebuildSignals: $('pRebuild').checked
    };

    try {
        const res = await fetch('/api/rtriangle/scan', {
            method: 'POST',
