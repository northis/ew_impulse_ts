...
namespace TradeKit.AlgoBase
{
    ...
    public class ElliottWavePatternFinder
    {
            ...
            BarPoint start, BarPoint end, out ElliottModelResult result)
        {
            ...
            KeyValuePair<DateTime, int> wave3End = (isUp ? highs : lows)
            ...
            IEnumerable<KeyValuePair<DateTime, double>> wave4And5 = (isUp 
                    ? m_PivotPointsFinder.LowValues 
                    : m_PivotPointsFinder.HighValues)
                .SkipWhile(a => a.Key <= wave3End.Key);
            ...
            KeyValuePair<DateTime, int> wave1End = (isUp ? highs : lows)
                .TakeWhile(a => a.Key < wave3End.Key)
                .MaxBy(a => a.Value);

            // This is real 1st wave
            // Or wave B (X) of wave 2 (check)
            IEnumerable<KeyValuePair<DateTime, double>> wave2And3 = (isUp
                    ? m_PivotPointsFinder.LowValues
                    : m_PivotPointsFinder.HighValues)
                .SkipWhile(a => a.Key <= wave1End.Key)
                .TakeWhile(a => a.Key < wave3End.Key);
            ...
            waves.Insert(1, new BarPoint(wave1End.Value, wave1End.Key, m_BarsProviderMain));
            waves.Insert(2, new BarPoint(wave2End.Value, wave2End.Key, m_BarsProviderMain));
            waves.Insert(3, new BarPoint(wave3End.Value, wave3End.Key, m_BarsProviderMain));
